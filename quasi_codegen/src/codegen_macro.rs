...
macro_rules! mk_lit {
    ($builder:expr, $name:expr, $suffix:expr, $($args:expr),+) => {{
...
macro_rules! module { ($module_path:expr) => {

use self::syntax::ast;
use self::syntax::codemap::Span;
use self::syntax::ext::base::ExtCtxt;
use self::syntax::ext::base;
...
pub fn expand_quote_tokens<'cx>(
    ...
    let expanded = expand_wrapper(sp, cx_expr, expr, &[&$module_path]);
...
pub fn expand_quote_matcher<'cx>(
    ...
    let expanded = expand_wrapper(sp, cx_expr, block, &[&$module_path]);
    base::MacEager::expr(expanded)
}
...
fn expr_mk_token(builder: &aster::AstBuilder, tok: &token::Token) -> P<ast::Expr> {
    match *tok {
        ...
        token::Literal(token::Str_(ident), suf) => {
            mk_lit!(builder, "Str_", suf, mk_name(builder, ident.ident()))
        }
